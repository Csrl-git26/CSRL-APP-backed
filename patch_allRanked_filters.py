...
# 1. Define filteredRanked (right after the state declarations or at the end of the imports)
# Actually, let's just insert it right after the `const filteredStudents = useMemo(...)` block to ensure `profileByRoll` is defined!

old_filteredStudents_end = """      const matchCat     = filterCategory === 'ALL' || p.CATEGORY   === filterCategory;
      const matchGender  = filterGender   === 'ALL' || p.GENDER     === filterGender;
      const matchState   = filterState    === 'ALL' || p['STATE (from where completing class 12th)'] === filterState;
      return matchSearch && matchSponsor && matchStream && matchCat && matchGender && matchState;
    });
  }, [data, searchTerm, filterSponsor, filterStream, filterCategory, filterGender, filterState]);"""

new_filteredStudents_end = """      const matchCat     = filterCategory === 'ALL' || p.CATEGORY   === filterCategory;
      const matchGender  = filterGender   === 'ALL' || p.GENDER     === filterGender;
      const matchState   = filterState    === 'ALL' || p['STATE (from where completing class 12th)'] === filterState;
      return matchSearch && matchSponsor && matchStream && matchCat && matchGender && matchState;
    });
...
if old_filteredStudents_end in content:
    content = content.replace(old_filteredStudents_end, new_filteredStudents_end)
else:
    print("Could not find old_filteredStudents_end")
...
with open(filepath, 'w') as f:
    f.write(content)
print("Patched CentreDashboard.jsx with filteredRanked, search filters, and Actions column")
